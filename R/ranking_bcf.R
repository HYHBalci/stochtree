...
#' bcf_model <- ranking_bcf(X_train = X_train, Z_train = Z_train, y_train = y_train, 
#'                  propensity_train = pi_train, X_test = X_test, Z_test = Z_test, 
#'                  propensity_test = pi_test, num_gfr = 10, 
#'                  num_burnin = 0, num_mcmc = 10)
#' @export
ranking_bcf <- function(X_train, Z_train, y_train, propensity_train = NULL, kl_divergence_method = c("distributional", "listnet"), rfx_group_ids_train = NULL, 
    ...
    kl_divergence_method <- match.arg(kl_divergence_method)
    if (kl_divergence_method == "listnet") {
        leaf_model_tau_forest <- 4
    } else {
        leaf_model_tau_forest <- 5
    }
    ...
    if (ncol(Z_train) > 1) {
        ...
        # Note: leaf_model_tau_forest is already set above
    ...
    # Data
    forest_dataset_train <- createUpliftForestDataset(X_train, Z_train, tau_basis_train)
    if (has_test) forest_dataset_test <- createUpliftForestDataset(X_test, Z_test, tau_basis_test)
    ...
    if (num_burnin + num_mcmc > 0) {
        for (chain_num in 1:num_chains) {
            ...
            for (i in (num_gfr+1):num_samples) {
                ...
                # Sample the treatment forest
                if (kl_divergence_method == "listnet") {
                    # Extract partial residuals (since outcome_train was mutated)
                    partial_resids <- outcome_train$get_data()
                    
                    treated <- which(Z_train == 1)
                    
                    # P_star from partial residuals
                    # Subtract max for numerical stability
                    max_pr <- max(partial_resids[treated])
                    exp_pr <- exp(partial_resids[treated] - max_pr)
                    P_star <- numeric(length(partial_resids))
                    P_star[treated] <- exp_pr / sum(exp_pr)
                    
                    # P from current tau predictions
                    tau_preds <- active_forest_tau$predict_raw(forest_dataset_train)
                    max_tau <- max(tau_preds[treated])
                    exp_tau <- exp(tau_preds[treated] - max_tau)
                    P <- numeric(length(tau_preds))
                    P[treated] <- exp_tau / sum(exp_tau)
                    
                    g_i <- numeric(length(tau_preds))
                    h_i <- numeric(length(tau_preds))
                    
                    g_i[treated] <- P[treated] - P_star[treated]
                    h_i[treated] <- P[treated] * (1 - P[treated])
                    
                    # Update dataset
                    outcome_train$update_data(g_i)
                    forest_dataset_train$update_weights(h_i)
                }
                ...
                # Restore outcome_train to partial residual if listnet modified it
                if (kl_divergence_method == "listnet") {
                    outcome_train$update_data(partial_resids)
                }
...
#'
#' @param object Object of type `ranking_bcfmodel` containing draws of a Bayesian causal forest model and associated sampling outputs.
...
#' @export
predict.ranking_bcfmodel <- function(object, X, Z, propensity = NULL, rfx_group_ids = NULL, rfx_basis = NULL, ...){
    # Preprocess covariates
    ...
    forest_dataset_pred <- createUpliftForestDataset(X_combined, rep(0, nrow(X_combined)), Z)
...
getRandomEffectSamples.ranking_bcfmodel <- function(object, ...){
    result = list()
