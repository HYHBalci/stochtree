...
double sample_alpha_cpp(const cpp11::writable::doubles& r_alpha, const cpp11::doubles& z, const cpp11::doubles& w, double sigma, double alpha_prior_sd) {
  int N = r_alpha.size();
...
cpp11::writable::list updateLinearTreatmentCpp_NCP_amr(
  ...
  if (gibbs) {
    ...
    if (use_bhatt_sampler) {
      ...
      Eigen::VectorXd delta(n);
      for (int i = 0; i < n; ++i) delta(i) = Rf_rnorm(0.0, 1.0 / std::sqrt(obs_weights[i])); // delta ~ N(0, W^{-1})
      
      Eigen::VectorXd v = X_star_scaled * u + delta; 
